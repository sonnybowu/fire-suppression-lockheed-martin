...
class my_flight_controller(student_base):
	...
	def add_fires(self):
		...
		self.fires = [Fire(lats[i], lons[i]) for i in range(len(lats))]
	...
	def move_to(self, telemetry, pos, label):
		lat = pos[0], lon = pos[1]
		print(f'Get to {label}')
		self.goto(lat, lon, self.ALT)
	...
	def student_run(self, telemetry, commands):
		...
		print("Arming"), self.arm()
		print("Taking off"), self.takeoff()
		...
		for _ in range(len(self.fires)):
			# Get water
			self.move_to(telemetry, (42.3608, -70.9904), 'water')
			time.sleep(6)

			# Put out fire
			next_fire = self.fires.pop( self.closest_fire(self.anchor) )
			self.move_to(telemetry, next_fire.centroid, 'fire')
			time.sleep(6)

		self.move_to(telemetry, home, 'base')
		time.sleep(10)
		print("Landing")
		self.land()
